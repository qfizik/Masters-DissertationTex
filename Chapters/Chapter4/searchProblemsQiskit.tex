...
Dispite the effects introduced by noise, the results are satisfactory when
considering the properties of NISQ computers. The following sections will
present the search problem adpated to several quantum walk models.

\subsection{Coined}
Following the structure of section \ref{sec:CoinedSearchSimul}, this chapter
aims to expand the coined quantum walk model incorporating concepts like the
oracle and diffusion operator of the Grover algoritm, the difference being the
creation of the circuit associatewith this alghorithm.\par 
...
of a complete graph, it is easily implemented using the aforemention
...
the aforemention $0.71$ when only Hadamard transformations are performed.\par
